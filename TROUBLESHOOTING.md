...
```bash
# Stage 1: Automated Unit & Invariant Test Suite (43 Tests)
...
- **Symptom:** `AssertionError: State vector $\vert\alpha\vert^2 + \vert\beta\vert^2 \ne 1$` during custom belief initialization.
...
| `EGE-003` | `0x103` | Quantum Engine | **State Vector Denormalization:** $\vert\alpha\vert^2 + \vert\beta\vert^2 \ne 1$. *Remedy: Use `QuantumBeliefState.from_confidence()`.* |
